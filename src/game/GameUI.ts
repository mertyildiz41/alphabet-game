...
export class GameUI {
    ...
    private starImage?: HTMLImageElement; // This will now represent coins
    private heartImage?: HTMLImageElement;
    private bowImage?: HTMLImageElement; // For the bow icon on the right

    constructor(assetsLoaded: () => void) {
        let loadedCount = 0;
    ...
    draw(context: CanvasRenderingContext2D, gameState: GameState, canvasWidth: number, canvasHeight: number) {
        const uiMargin = 20;
        const iconSize = 30;
        ...
        if (this.avatarImage && this.avatarImage.complete && this.avatarImage.naturalHeight !== 0) {
            ...
            context.fillStyle = '#4CAF50'; context.beginPath(); context.arc(uiMargin + iconSize*0.75, uiMargin + iconSize*0.75, iconSize*0.75, 0, Math.PI*2); context.fill();
            context.fillStyle = 'white'; context.font = 'bold 20px Arial'; context.textAlign = 'center'; context.textBaseline = 'middle';
        ...
        } else { context.strokeStyle = '#D2691E'; context.lineWidth = 2; context.strokeRect(canvasWidth - uiMargin - iconSize, bowIconY, iconSize, iconSize); context.fillText("B", canvasWidth - uiMargin - iconSize/2, bowIconY + iconSize/2); }
