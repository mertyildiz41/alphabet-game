import { Player, Letter as LetterEntity, Projectile } from '../entities/index.js';
...
export class Game {
    ...
    private letterSpawnInterval: number = 1800; // ms, slightly faster
    ...
    private getInitialGameState(): GameState {
        return {
            score: 0, // Score might be repurposed or removed if coins are the primary currency
            lives: 3,
            targetLetter: "", // No specific target letter initially, or remove if not used
            isGameOver: false,
    ...
    private setupInputHandlers(): void {
        this.canvas.addEventListener('mousemove', (e) => {
            if (this.gameState.isGameOver && this.assetsReady) return;
    ...
    private update(deltaTime: number): void {
        ...
        for (let i = this.projectiles.length - 1; i >= 0; i--) {
            ...
            for (let j = this.letters.length - 1; j >= 0; j--) {
                ...
                if (!letter.isActive || letter.isHit) continue;
        ...
        for (let i = this.letters.length - 1; i >= 0; i--) {
            ...
            if (!letter.isActive) { 
                if (!letter.isHit) { 
                    ...
                    if (this.gameState.lives < 0) this.gameState.lives = 0; // Prevent negative lives display
                }
