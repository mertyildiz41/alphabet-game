...
function initializeAlphabetGame() {
    if (!alphabetGameContainer) return;
    // Clear previous canvas if any
    alphabetGameContainer.innerHTML = ''; 
    const canvas = document.createElement('canvas');
    alphabetGameContainer.appendChild(canvas);
    const context = canvas.getContext('2d');
    if (!context) throw new Error("Failed to get 2D context for Alphabet Shooter.");
...
function initializeSnakeGame() {
    ...
    const context = canvas.getContext('2d');
    if (!context) throw new Error("Failed to get 2D context for Snake Game.");
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
...
function initializeAlphabetArrowGame() {
    if (!alphabetArrowGameContainer) return;
    alphabetArrowGameContainer.innerHTML = '';
    const canvas = document.createElement('canvas');
    alphabetArrowGameContainer.appendChild(canvas);
    const context = canvas.getContext('2d');
    if (!context) throw new Error("Failed to get 2D context for Alphabet Arrow Game.");
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
...
if (alphabetShooterGameBox && landingPage && alphabetGameContainer) {
    alphabetShooterGameBox.addEventListener('click', () => {
        ...
        if (alphabetArrowGameContainer) alphabetArrowGameContainer.classList.add('hidden');
...
if (snakeGameBox && landingPage && snakeGameContainer) {
    snakeGameBox.addEventListener('click', () => {
        ...
        if (alphabetGameContainer) alphabetGameContainer.classList.add('hidden');
        if (alphabetArrowGameContainer) alphabetArrowGameContainer.classList.add('hidden');
        document.body.style.overflow = 'hidden';
        initializeSnakeGame();
    });
